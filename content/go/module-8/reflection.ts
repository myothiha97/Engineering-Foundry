...
export const goReflection: Lesson = {
  ...
  sections: {
    ...
    "mental-model": {
      body: "Rob Pike explains reflection with three laws.\n\n**1. Interface value → reflection object.** Pass a value to `reflect.TypeOf` or `reflect.ValueOf` to get a `reflect.Type` or `reflect.Value`.\n\n**2. Reflection object → interface value.** Calling `value.Interface()` returns an `any` that you can type-assert back to a concrete type. The round trip preserves the original dynamic value and type.\n\n**3. A value must be settable before reflection can modify it.** In practice, pass a pointer and use `.Elem()` to reach the value it points to. A plain `reflect.ValueOf(x)` holds a non-settable copy, so calling `Set` on it panics.\n\nRemember the third law in particular; it explains the most common reflection mistake.",
      blocks: [
        {
          type: "note",
          note: {
            ...
            nodes: [
              {
                ...
                label: "reflect.Type",
                detail:
                  "The exact declared type: main.Transaction, main.Celsius, *os.File. Distinct per declaration.",
                tone: "accent",
              },
              {
                id: "kind",
                label: "reflect.Kind",
                detail:
                  "The underlying category: Struct, Slice, Ptr, Int, Float64, String. Many Types share one Kind.",
                tone: "success",
              },
            ],
            caption:
              'Ask Type for identity ("which type exactly?"); ask Kind for shape ("is it a struct/slice/pointer?").',
          },
        },
        {
          type: "example",
    ...
    diagram: {
      body: "Now the part that trips up everyone: **settability**. Reading through reflection is usually straightforward; writing has a stricter rule. Call `v.CanSet()` before a Set method when the value may come from unknown input. `reflect.ValueOf(x)` holds a non-settable copy, while `reflect.ValueOf(&x).Elem()` reaches the original storage and is settable. Export rules still apply: an unexported struct field is not settable.\n\nFollow the steps below: a plain value gives an unsettable Value (Set panics); a pointer's `.Elem()` gives a settable Value (Set works).",
      blocks: [
        {
          type: "diagram",
          diagram: {
            ...
            nodes: [
              {
                id: "plain",
                label: "reflect.ValueOf(x)",
                detail: "copies x — the Value is not addressable",
              },
              {
                id: "setpanic",
                label: "v.SetString(...)",
                detail: "PANIC: using unaddressable value",
                tone: "danger",
              },
              {
                id: "ptr",
                label: "reflect.ValueOf(&x)",
                detail: "a Value of Kind Ptr, pointing at x's storage",
              },
              {
                id: "elem",
                label: ".Elem()",
                detail: "follows the pointer to the addressable x",
                tone: "accent",
              },
              {
                id: "setok",
                label: "v.SetString(...)",
                detail: "works — x itself is now changed",
                tone: "success",
              },
            ],
            caption:
              "No pointer, no Set. Reflection can only write where there is real storage to write to.",
          },
        },
        {
          type: "note",
      ...
      blocks: [
        {
          ...
          items: [
            ...
            "**Calling Set without checking generic input** → Set methods panic when the value is not settable. Check `v.CanSet()` first.",
      ...
      blocks: [
        {
          ...
          items: [
            ...
            "If you must reflect, isolate it, check `Kind()` before shape-specific calls and `CanSet()` before writes, then test it hard.",
          ],
        },
        {
          type: "scenario",
    ...
    summary: {
      body: "Two ideas carry this lesson. **Reflection reads the pair an interface already holds** — you hand a value to `reflect.TypeOf`/`reflect.ValueOf`, get back a Type and a Value, inspect Kind and fields, read `json` tags, and (through a pointer's `. Elem()`) even Set. That's how `encoding/json` marshals a struct it was never compiled against.\n\n**But reflection costs safety and speed** — it defeats the type checker so bugs surface at runtime, and it's slow because there's no compile-time dispatch — so reach for interfaces, generics, or code generation first, and keep reflection confined to libraries like codecs and validators.",
      blocks: [
        {
          type: "points",
          items: [
