  ...
  references: [
    {
      ...
      teaches:
        "Why an interface value can be non-nil even when the concrete value it holds is a nil pointer.",
      relevance:
        "The official, minimal demonstration of the exact typed-nil behavior this lesson centers on.",
      required: false,
    ...
    },
    {
      ...
      teaches:
        "Idiomatic interface design: implicit satisfaction and the preference for small interfaces.",
      relevance:
        "Backs the 'keep interfaces small and let types satisfy them implicitly' design guidance.",
      required: false,
    ...
    },
    {
      ...
      teaches:
        "The precise definition of an interface type and how a type's method set determines what it satisfies.",
      relevance:
        "The authoritative source for method sets and what it means to implement an interface.",
      required: false,
  ...
  exercises: [
    {
      ...
      hints: [
        "An interface value is a pair: a concrete type and a value.",
        "Which half of the pair is nil, and which is not?",
      ],
    },
    {
      ...
      hints: [
        ...
        "If a type has all of an interface's methods, it satisfies that interface automatically.",
      ],
    },
    {
    ...
    },
    {
      ...
      hints: [
        "A value of type T does not include methods that have a pointer receiver.",
        "Assign a *Account (e.g. `&Account{}`), not an Account.",
      ],
    },
    {
      ...
      hints: [
        "Prefer small interfaces — a one-method interface like io.Writer is idiomatic.",
        "Define the interface where it is consumed, listing only the methods used.",
      ],
    },
    {
      ...
      type: "design",
      prompt:
        "Design a minimal TaskStore interface for a service that needs to load and save tasks, and justify why the service should depend on it rather than on a concrete database type.",
      hints: [
        "List only the methods the service actually calls.",
        "What does depending on behavior instead of a concrete type let you swap in tests?",
      ],
    },
    {
      ...
      hints: [
        "Returning a typed nil pointer as an interface stores a non-nil type descriptor.",
        "Return the untyped `nil` literal directly on the success path instead of a nil typed pointer.",
      ],
    },
  ],
  masteryCriteria: [
    {
      ...
      kind: "explain",
      description:
        "Explain what an interface is (a set of method signatures) and why Go satisfies it implicitly, with no `implements` keyword.",
      required: true,
    },
    {
      ...
      kind: "predict",
      description:
        "Predict from a method's receiver whether T or only *T satisfies a given interface.",
      required: true,
    },
    {
      id: "explain-typednil",
      kind: "explain",
      description:
        "Explain why an interface holding a nil typed pointer is itself not equal to nil, using the (type, value) pair.",
      required: true,
    },
    {
      id: "design-interface",
      kind: "design",
      description: "Design a small, consumer-side interface and defend keeping it minimal.",
      required: false,
    },
  ],
  sections: {
      ...
      blocks: [
        ...
        },
        {
          ...
          example: {
            ...
            language: "go",
            code: "// The whole io.Writer contract — one method.\ntype Writer interface {\n    Write(p []byte) (n int, err error)\n}\n\n// A file, a network connection, and a bytes.Buffer all have Write,\n// so all three satisfy Writer with zero extra work.\nfunc save(w Writer, data []byte) error {\n    _, err := w.Write(data)\n    return err\n}",
            takeaway:
              "A one-method interface is satisfied by everything that can do that one thing — maximum reuse.",
          },
        },
      ],
    },
    mechanics: {
      body: "Now the precise pieces. You declare an interface with `type Name interface { ... }`, listing method signatures (name, parameters, results) and no bodies. A type satisfies it by having methods whose signatures match — that's the whole rule.\n\nThe subtlety is the **method set**: the exact list of methods a given type 'has' for interface purposes. A value of type `T` has only the methods declared with a **value receiver** (`func (x T) M()`). A pointer `*T` has **both** value-receiver *and* pointer-receiver methods (`func (x *T) M()`).\n\nSo if a method uses a pointer receiver, only `*T` — not `T` — carries it, and only `*T` satisfies an interface that needs it. Trying to assign a `T` value there is the classic compile error `T does not implement I (M method has pointer receiver)`.",
      blocks: [
        {
          type: "example",
          example: {
      ...
      blocks: [
        {
          ...
          diagram: {
            ...
            nodes: [
              {
                id: "s1",
                label: "var p *StoreError = nil",
                detail: "p is a nil pointer of a concrete type",
              },
              {
                id: "s2",
                label: "var err error = p",
                detail: "interface gets type=*StoreError, value=nil",
                tone: "accent",
              },
              {
                id: "s3",
                label: "err == nil ?",
                detail: "type slot is filled → comparison is FALSE",
                tone: "danger",
              },
              {
                id: "s4",
                label: "if err != nil { ... }",
                detail: "branch fires on a successful call — the phantom error",
                tone: "danger",
              },
            ],
            caption:
              "Assigning a typed nil pointer to an interface fills the type slot, so the interface is not nil.",
          },
        },
        {
          type: "note",
      ...
      blocks: [
        {
          ...
          example: {
            ...
            takeaway:
              "`send` works with any Notifier — swap in an SMSNotifier tomorrow and nothing in send changes.",
          },
        },
        {
          type: "example",
    ...
    experiment: {
      body: "Predict before you read on — a corrected wrong guess sticks better than a right answer you skim. Consider this program:\n\n`type I interface{ M() }`\n`type T struct{}`\n`func (t *T) M() {}`\n`var p *T = nil`\n`var i I = p`\nthen `fmt.Println(i == nil)`.\n\nWhat prints — `true` or `false`? Commit to an answer.\n\nIt prints **false**. Even though `p` is a nil pointer, assigning it to the interface `i` stored two things: the concrete type `*T` in the type slot and the nil pointer in the value slot. An interface equals `nil` only when *both* slots are empty — here the type slot holds `*T`, so `i` is not nil.\n\nChange the program to `var i I` with no assignment and `i == nil` becomes **true**, because now neither slot was ever filled. The single act of assigning a typed value — even a nil one — is what makes the interface non-nil. That is the entire typed-nil trap.",
    },
    "failure-cases": {
      ...
      blocks: [
        ...
        },
        {
          ...
          example: {
            ...
            takeaway:
              "If the method has a pointer receiver, only `*T` satisfies the interface — assign a pointer.",
          },
        },
      ],
    },
    ...
    design: {
      ...
      blocks: [
        ...
        },
        {
          type: "scenario",
          scenario: {
            title: "Designing a small storage interface",
            context:
              "A task service needs only Get and Save. The concrete database store has fifteen methods.",
            insight:
              "Define a two-method TaskStore next to the service. The database store and an in-memory fake both satisfy it implicitly, while the service stays independent of either implementation.",
          },
        },
      ],
    },
