...
export const goIoReaderWriter: Lesson = {
  ...
  ledgerFlowApplications: [
    "Stream a large CSV export straight to the HTTP response without buffering every row in memory",
    "Wrap the export writer in io.MultiWriter to send it to the client and an audit log at once",
    "Accept an io.Reader for imports so the same parser handles a file upload, a test string, and a network body",
  ],
  references: [
    {
      title: "package io",
      url: "https://pkg.go.dev/io",
      teaches: "The Reader and Writer interfaces, the io.EOF sentinel, and helpers like io.Copy, io.MultiWriter, and io.TeeReader.",
      relevance: "The authoritative definition of the two interfaces and the composition helpers this lesson is built on.",
      required: true,
    ...
    },
    {
      ...
      teaches: "Why Go favors small interfaces and satisfies them implicitly, using io.Writer as the canonical example.",
      relevance: "Explains why one-method interfaces like Reader and Writer are so widely reusable across the standard library.",
      required: true,
    ...
    },
    {
      ...
      teaches: "How bufio.NewReader and bufio.NewWriter wrap another Reader/Writer to add buffering and line-oriented reads.",
      relevance: "Backs the composition section, where wrapping a Reader with a Reader is the everyday move.",
      required: false,
  ...
  exercises: [
    {
      ...
      hints: ["io.EOF is a normal end-of-stream signal, not a failure.", "Always use the returned n bytes before you inspect err."],
    },
    {
      id: "go5io-read-copy",
      type: "code-reading",
      ...
      hints: ["io.Copy takes an io.Writer and an io.Reader, not concrete types.", "Every one of those types has a Read or Write method with the right signature."],
    },
    {
    ...
    },
    {
      ...
      hints: ["Read fills only the first n bytes of the buffer; the rest is leftover from the previous read.", "Always slice the buffer to the count Read actually returned: dst.Write(buf[:n])."],
    },
    {
      ...
      hints: ["io.Copy moves bytes through a small fixed buffer, so peak memory is the buffer size, not the file size.", "You rarely need the whole payload in memory at once to move it from source to destination."],
    },
    {
      id: "go5io-design-export",
      type: "design",
      prompt:
        "Design the signature and data flow for a LedgerFlow transaction export that must (a) stream to the HTTP client without buffering all rows and (b) simultaneously write a copy to an audit sink. Say which io helper ties the two destinations together and why the writer type should be an interface.",
      hints: ["io.MultiWriter(a, b) returns a Writer that fans every write out to both a and b.", "Accepting io.Writer lets the same code target an HTTP response in production and a bytes.Buffer in tests."],
    },
    {
  ...
  sections: {
    ...
    naive: {
      body: "The beginner instinct is to type functions to concrete things. You write `func process(f *os.File)` because today your input happens to be a file. It compiles, it runs, everyone's happy — until a teammate wants to feed it an HTTP request body, or you want to test it with a hard-coded string, and neither of those is an `*os.File`.\n\nThe second naive move is to reach for `os.ReadFile` or `io.ReadAll` and slurp the *entire* input into one big `[]byte` before doing anything. For a 2 KB config that's fine. For a 4 GB export it's a memory bomb — you hold the whole payload in RAM just to pass it along.",
      blocks: [
        {
          type: "example",
          example: {
            title: "Typed to a file — and stuck with files",
            language: "go",
            code:
              'func countBytes(f *os.File) (int, error) {\n    data, err := io.ReadAll(f) // reads the ENTIRE file into memory\n    return len(data), err\n}\n\n// Cannot pass an http.Request Body, a strings.Reader, or a bytes.Buffer here —\n// they are not *os.File, even though every one of them can be read.',
            takeaway: "Naming a concrete type locks the function to that type and forces you to buffer everything at once.",
          },
        },
        {
          type: "points",
          items: [
            "Typing to `*os.File` rejects every other byte source, including test inputs.",
            "Reading everything into one `[]byte` uses memory proportional to the input size.",
          ],
        },
      ],
    },
    failure: {
      body: "The concrete-type version doesn't fail loudly — it fails by *painting you into a corner*. The day requirements change, you either rewrite the function's signature and everything that calls it, or you copy the logic into a near-identical second function. Both are the kind of quiet duplication that rots a codebase.\n\nThe read-everything version fails more dramatically. Under load, a handful of large concurrent uploads or exports each hold their full payload in memory at the same time, and the process is killed for running out of memory — an OOM crash that never shows up on the small files you tested with.",
      blocks: [
        {
          type: "scenario",
          scenario: {
            title: "The export that killed the server",
            context:
              "An export endpoint calls io.ReadAll on the full result set, builds one giant []byte, then writes it to the response. It sails through tests on a 5 MB sample. In production a few users export a year of transactions at once — several hundred MB each, held in memory simultaneously — and the container is OOM-killed.",
            insight: "Nothing was 'wrong' with the code; it just chose to hold the whole payload in memory. Streaming through a small buffer would have used a few kilobytes regardless of export size.",
          },
        },
        {
          type: "points",
          items: [
            "Concrete parameter types force rewrites or copy-paste when the source changes.",
            "Buffering the whole input scales memory with input size — invisible until a big input arrives.",
          ],
        },
      ],
    },
    intuition: {
      body: "Both problems dissolve with one shift: depend on *behavior*, not on a *type*, and move bytes in *small chunks* rather than all at once. 'Behavior' here is captured by the two interfaces — a `Reader` is anything with a `Read` method, a `Writer` is anything with a `Write` method — so your function names the interface and accepts every type that qualifies.\n\n'Small chunks' is what **streaming** means: pull a bufferful of bytes, hand it onward, repeat, until the source is exhausted. A tiny fixed buffer shuttles a 4 GB file just as happily as a 4 KB one, because the file is never all in memory at once — only the current chunk is.",
      blocks: [
        {
          type: "diagram",
          diagram: {
            title: "Streaming through a small buffer",
            kind: "flow",
            nodes: [
              { id: "src", label: "Reader", detail: "file / conn / string / body" },
              { id: "buf", label: "small buffer", detail: "e.g. 32 KB, reused every loop", tone: "accent" },
              { id: "dst", label: "Writer", detail: "file / response / buffer" },
            ],
            caption: "The same buffer is refilled and drained in a loop, so peak memory is the buffer size, not the stream size.",
          },
        },
        {
          type: "points",
          items: [
            "A Reader is 'anything with Read'; a Writer is 'anything with Write' — depend on the method, not the type.",
            "Streaming moves bytes chunk by chunk, so memory stays flat no matter how large the stream is.",
            "One interface-typed function serves files, sockets, strings, and HTTP bodies alike.",
          ],
        },
      ],
    },
    "mental-model": {
      body: "Keep three ideas and the whole package clicks. **One:** the two interfaces really are this small — `Reader` has exactly one method, `Read`, and `Writer` has exactly one, `Write`. Small interfaces are easy to satisfy, so *everything* satisfies them. **Two:** a byte stream is not a value you hold; it's a faucet you pull from until it runs dry, and `io.EOF` is the sound of the faucet running dry — a normal signal, not a leak. **Three:** because both ends are interfaces, readers and writers **compose** — you can wrap a Reader in another Reader to add a feature, the way you'd screw an adapter onto that universal pipe fitting.\n\nThat third idea is the real superpower. `io.Copy` glues any Reader to any Writer; `bufio.NewReader` wraps a Reader to add buffering; `io.MultiWriter` bundles several Writers into one; `io.TeeReader` wraps a Reader so reading also writes a copy elsewhere. Each is just a Reader or Writer that holds another Reader or Writer.",
      blocks: [
        {
          type: "example",
          example: {
            ...
            takeaway: "One method each. Anything implementing Read is a Reader; anything implementing Write is a Writer — no registration, no keyword.",
          },
        },
        {
          type: "note",
      ...
      blocks: [
        {
          ...
          example: {
            ...
            takeaway: "Process buf[:n] first, then check err. io.EOF ends the loop cleanly; any other error is a genuine failure.",
          },
        },
        {
          type: "points",
      ...
      blocks: [
        {
          ...
          diagram: {
            ...
            kind: "sequence",
            nodes: [
              { id: "r1", label: "Read(buf) → n=4, err=nil", detail: "buffer filled with the first 4 bytes; more remain" },
              { id: "r2", label: "Read(buf) → n=1, err=nil", detail: "a short read: only 1 byte left fit; still not the end", tone: "accent" },
              { id: "r3", label: "Read(buf) → n=0, err=io.EOF", detail: "no bytes left; io.EOF signals the stream is drained", tone: "danger" },
              { id: "stop", label: "loop stops on io.EOF", detail: "clean end — all 5 bytes were processed, nothing failed", tone: "success" },
            ],
            caption: "Short reads (n < len(buf)) are normal. io.EOF is the signal to stop, not a sign something broke.",
          },
        },
        {
          type: "note",
      ...
      blocks: [
        {
          ...
          example: {
            ...
            takeaway: "io.Copy(dst, src) is the streaming copy you'd otherwise write by hand — and it works for any Reader/Writer pair.",
          },
        },
        {
          type: "points",
      ...
      blocks: [
        ...
        },
        {
          ...
          example: {
            ...
            takeaway: "On the final read n might be 10, but Write(buf) still emits 512 bytes — 502 of them stale. Always Write(buf[:n]).",
          },
        },
      ],
    },
      ...
      blocks: [
        ...
        },
        {
          ...
          scenario: {
            ...
            context: "You must save an upload to disk and compute its checksum, and you'd rather not read the bytes twice.",
            insight: "Wrap the source in io.TeeReader(src, hasher), then io.Copy(file, tee). Each byte flows to the file and the hasher in one pass — composition instead of a second read.",
          },
        },
      ],
    },
    ledgerflow: {
      body: "This is exactly how LedgerFlow handles bulk data. A transaction export streams rows straight into the HTTP response through `io.Copy`, so exporting a year of history uses a few kilobytes of buffer instead of holding every row in memory — the OOM crash from the failure section simply can't happen. Because the export function takes an `io.Writer`, the same code writes to a real response in production and to a `bytes.Buffer` in tests, with nothing swapped but the argument.\n\nComposition earns its keep on the audit requirement: every export must also be copied to an audit sink. Instead of writing the data twice, LedgerFlow wraps the two destinations in `io.MultiWriter(response, auditLog)` and writes once — the bytes fan out to both. On the import side, the CSV parser accepts an `io.Reader`, so a browser upload, a test string, and a network body all flow through the identical parsing path.",
      blocks: [
        {
          type: "example",
          example: {
            title: "Stream an export to the client and the audit log at once",
            language: "go",
            code:
              'import (\n    "io"\n    "net/http"\n)\n\n// rows is an io.Reader that yields encoded transactions on demand;\n// audit is an io.Writer for the audit sink.\nfunc exportTransactions(w http.ResponseWriter, rows io.Reader, audit io.Writer) error {\n    // One Writer that fans every byte out to the client AND the audit log.\n    sink := io.MultiWriter(w, audit)\n\n    // Stream chunk by chunk — memory stays flat no matter how many rows.\n    _, err := io.Copy(sink, rows)\n    return err\n}',
            takeaway: "io.MultiWriter + io.Copy stream the export to two destinations in one pass, with memory independent of export size.",
          },
        },
        {
          type: "diagram",
          diagram: {
            title: "Export fans out through io.MultiWriter",
            kind: "flow",
            nodes: [
              { id: "rows", label: "rows (Reader)", detail: "encoded transactions, streamed" },
              { id: "copy", label: "io.Copy", detail: "chunk by chunk", tone: "accent" },
              { id: "multi", label: "io.MultiWriter", detail: "one Writer, two targets" },
              { id: "client", label: "HTTP response", detail: "the download", tone: "success" },
              { id: "audit", label: "audit log", detail: "the mandated copy", tone: "success" },
            ],
            caption: "A single streamed copy reaches both the client and the audit log; neither buffers the whole export.",
          },
        },
      ],
    },
    exercises: {
      body: "Practice turns 'I recognize this' into 'I can predict and build it'. Work across prediction, code-reading, implementation, debugging, refactoring, design, and one advanced composition puzzle. Each kind produces different evidence, so clearing one doesn't cover the rest — the io.EOF prediction and the streaming-copy implementation are the two to do before you trust yourself with real I/O.",
    },
    mastery: {
      ...
      blocks: [
        {
          ...
          items: [
            ...
            "Next up: errors as values and how Go models failure explicitly.",
          ],
        },
      ],
    },
